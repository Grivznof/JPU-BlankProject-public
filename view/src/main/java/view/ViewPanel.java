...
class ViewPanel extends JPanel implements Observer {
	...
	protected void paintComponent(final Graphics graphics) {
		...
		try {
			imageTemp = ImageIO.read(new File("C:\\Users\\HUGO\\Desktop\\JPU-BlankProject-public1\\sprites" + "\\" + "background.png"));
		} catch (IOException e) {
			e.printStackTrace();
		}
		...
		for (int i = 0; i < DrawMap.size(); i++) {
			...
			try {
				if (DrawMap.get(i).getFactory().equals("nothing")) {
					...
					imageTemp = ImageIO.read(new File("C:\\Users\\HUGO\\Desktop\\JPU-BlankProject-public1\\sprites\\" + DrawMap.get(i).getFactory() + ".png"));
				} else {
					imageTemp = ImageIO.read(new File("C:\\Users\\HUGO\\Desktop\\JPU-BlankProject-public1\\sprites\\" + DrawMap.get(i).getFactory() + ".png"));
				}
